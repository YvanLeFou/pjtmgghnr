...
(function(window, undefined) {
		...
		jQuery
				.extend({
					...
					sub : function() {
						...
						jQuerySubclass.fn.init = function init(selector,
								context) {
	...
	(function() {
		...
		var all = div.getElementsByTagName("*"), a = div
				...
				.createElement("option")), input = div
		...
		jQuery.support.scriptEval = function() {
			if (_scriptEval === null) {
				var root = document.documentElement, script = document
						.createElement("script"), id = "script" + jQuery.now();
		...
		jQuery(function() {
			var div = document.createElement("div"), body = document
			...
			if (document.defaultView && document.defaultView.getComputedStyle) {
				...
				jQuery.support.reliableMarginRight = (parseInt(
						document.defaultView.getComputedStyle(div, null).marginRight,
						10) || 0) === 0;
			}
			...
			if (!eventHandle) {
				elemData.handle = eventHandle = function(e) {
					...
					return typeof jQuery !== "undefined"
							&& jQuery.event.triggered !== e.type ? jQuery.event.handle
					...
					if (jQuery.event.global[type]) {
						...
						jQuery
								.each(
										jQuery.cache,
										function() {
											// internalKey variable is just used
											// to make it easier to find
											// and potentially change this stuff
											// later; currently it just
	...
	jQuery.fn.extend({
		...
		add : function(selector, context) {
			var set = typeof selector === "string" ? jQuery(selector, context)
					: jQuery.makeArray(selector), all = jQuery.merge(
					this.get(), set);
	...
	rlocalProtocol = /^(?:about|app|app\-storage|.+\-extension|file|widget):$/, rnoContent = /^(?:GET|HEAD)$/, rprotocol = /^\/\//, rquery = /\?/, rscript = /<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, rselectTextarea = /^(?:select|textarea)/i, rspacesAjax = /\s+/, rts = /([?&])_=[^&]*/, rucHeaders = /(^|\-)([a-z])/g, rucHeadersFunc = function(
			_, $1, $2) {
				...
				ajax : function(url, options) {
					...
					jqXHR = {
						...
						setRequestHeader : function(name, value) {
							if (!state) {
								requestHeaders[name.toLowerCase().replace(
										rucHeaders, rucHeadersFunc)] = value;
							}
							return this;
						},
					...
					// dataType
					requestHeaders.Accept = s.dataTypes[0]
					...
					function(s, originalSettings, jqXHR) {
						...
						if (s.dataTypes[0] === "jsonp"
							...
							var responseContainer, jsonpCallback = s.jsonpCallback = jQuery
									...
									+ jsonpCallback + "$2", cleanUp = function() {
			...
			.each(
					...
					function(i, name) {
						...
						jQuery.fn["inner" + name] = function() {
							return this[0] ? parseFloat(jQuery.css(this[0],
									type, "padding")) : null;
						};
						...
						jQuery.fn["outer" + name] = function(margin) {
							return this[0] ? parseFloat(jQuery.css(this[0],
									type, margin ? "margin" : "border")) : null;
						};
